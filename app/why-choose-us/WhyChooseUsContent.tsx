...
function SMBVisual() {
  return (
    <svg viewBox="0 0 280 140" fill="none" className="w-full h-full">
      ...
      {/* Growth arrow path */}
      <path d="M155 100 Q175 85 195 75 Q215 65 235 40" stroke="url(#smb-wcu)" strokeWidth="2.5" strokeLinecap="round" fill="none" />
      <path d="M230 37 L237 40 L233 48" stroke="#14b8a6" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" fill="none" opacity="0.5" />
      {/* Growth bars */}
      {[0, 1, 2, 3, 4].map(i => (
        <rect key={i} x={155 + i * 18} y={120 - (i + 1) * 10} width="10" height={(i + 1) * 10}
...
function DualVisual() {
  return (
    <svg viewBox="0 0 280 140" fill="none" className="w-full h-full">
      ...
      {/* Connection bridge */}
      <line x1="95" y1="63" x2="175" y2="63" stroke="url(#dual-wcu)" strokeWidth="2" opacity="0.15" />
      <path d="M95 63 Q135 40 175 63" stroke="url(#dual-wcu)" strokeWidth="1.5" strokeDasharray="4 3" fill="none" opacity="0.1" />
      <path d="M95 63 Q135 86 175 63" stroke="url(#dual-wcu)" strokeWidth="1.5" strokeDasharray="4 3" fill="none" opacity="0.1" />
      {/* Center hub */}
      <circle cx="135" cy="63" r="8" fill="url(#dual-wcu)" opacity="0.12" />
      <circle cx="135" cy="63" r="4" fill="url(#dual-wcu)" opacity="0.25" />
      {/* SF cloud group */}
      <g>
...
function TransparentVisual() {
  return (
    <svg viewBox="0 0 280 140" fill="none" className="w-full h-full">
      {/* Open padlock */}
      <g>
        <path d="M90 52 L90 38 C90 24 105 14 120 14 C135 14 150 24 150 38 L150 42" stroke="#2563eb" strokeWidth="3" strokeLinecap="round" fill="none" opacity="0.15" />
        ...
        {/* Checkmark */}
        <path d="M105 78 L115 88 L140 63" stroke="#14b8a6" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" fill="none" />
      </g>
      {/* Document pages */}
      <g>
        ...
        {[0, 1, 2, 3].map(i => (
          <g key={i}>
            ...
            <path d={`M183.8 ${49.5 + i * 14} L185 ${51 + i * 14} L187 ${49 + i * 14}`}
    ...
    <svg viewBox="0 0 280 140" fill="none" className="w-full h-full">
      ...
      {[
        ...
        <g key={i}>
          ...
          {card.x >= 188 && (
            <path d={`M${card.x + card.w - 12} ${card.y + card.h / 2 - 1} L${card.x + card.w - 10} ${card.y + card.h / 2 + 1} L${card.x + card.w - 7} ${card.y + card.h / 2 - 2}`}
...
function PartnerVisual() {
  return (
    <svg viewBox="0 0 280 140" fill="none" className="w-full h-full">
      ...
      {/* Connection arcs */}
      <path d="M105 60 Q140 35 175 60" stroke="url(#part-wcu)" strokeWidth="2" strokeDasharray="4 3" fill="none" opacity="0.1" />
      <path d="M105 60 Q140 85 175 60" stroke="url(#part-wcu)" strokeWidth="2" strokeDasharray="4 3" fill="none" opacity="0.1" />
      {/* Center merge */}
      <circle cx="140" cy="60" r="8" fill="url(#part-wcu)" opacity="0.1" />
      <circle cx="140" cy="60" r="4" fill="url(#part-wcu)" opacity="0.25" />
      {/* Outcome metrics */}
      {[
    ...
    <>
      ...
      <section className="pb-6 pt-2">
        <div className="container-apple">
          <FadeIn>
            <div className="grid grid-cols-3 gap-4 rounded-[22px] bg-apple-black p-6 md:p-8">
              {stats.map((stat, i) => (
                <div key={stat.label} className={`text-center ${i < 2 ? "border-r border-white/10" : ""}`}>
                  <p className="text-[clamp(1.5rem,3vw,2.25rem)] font-bold text-gradient leading-none">
                    {stat.value}
                  </p>
